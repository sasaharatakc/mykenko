...
export default function VendorEditProductPage() {
  ...
  const { data: product, isLoading } = useQuery({
    ...
    queryFn:  () => vendorApi.showProduct(Number(id)).then(r => r.data?.data),
  })
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        attribute_set_ids: product.attribute_sets?.map((s: any) => s.id) ?? [],
      })
    ...
    mutationFn: (data: FormData) => {
      ...
      Object.entries(data).forEach(([k, v]) => {
        ...
        if ((k === 'category_ids' || k === 'attribute_set_ids') && Array.isArray(v)) {
          v.forEach(catId => fd.append(`${k}[]`, String(catId)))
        } else {
      ...
      return vendorApi.updateProduct(Number(id), fd)
    },
    onSuccess: () => {
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
              ...
              <div>
                ...
                <input type="number" step="0.01" {...register('weight')} className="w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
            </div>
          </div>
