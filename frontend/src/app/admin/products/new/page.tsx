...
export default function AdminNewProductPage() {
  ...
  const { data: specTablesData } = useQuery({
    ...
    queryFn:  () => adminApi.specTables({ per_page: 100 }).then(r => r.data?.data ?? []),
    staleTime: 5 * 60_000,
  })
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => {
      ...
      return adminApi.createProduct(fd)
    },
    onSuccess: () => { toast.success('商品を作成しました'); router.push('/admin/products') },
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" {...register('weight')} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
              ...
              <div>
                ...
                <select {...register('specification_table_id')} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20">
                  ...
                  {(specTablesData as any[] ?? []).map((t: any) => (
