...
export default function AdminEditBlogPage() {
  ...
  const { data: postData, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.showBlog(Number(id)).then((r) => r.data?.data),
  })
  ...
  useEffect(() => {
    if (postData) {
      ...
      if (postData.image) setImagePreview(getImageUrl(postData.image))
    }
  }, [postData, reset])
  ...
  function buildFormData(data: FormData): globalThis.FormData {
    ...
    Object.entries(data).forEach(([k, v]) => {
      ...
      if (k === 'category_ids' && Array.isArray(v)) {
        v.forEach((cid) => fd.append('category_ids[]', String(cid)))
      } else {
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => adminApi.updateBlog(Number(id), buildFormData(data)),
    onSuccess: () => {
  ...
  if (isLoading) return <div className="card p-6 h-48 skeleton" />

  return (
    <div className="space-y-5 max-w-2xl">
      <div className="flex items-center gap-3">
        <Link href="/admin/blog" className="btn-icon w-8 h-8 text-gray-500"><ArrowLeft className="w-4 h-4" /></Link>
        <h1 className="font-display text-2xl font-bold text-gray-900">Edit Post</h1>
      </div>
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5">
          ...
          {imagePreview ? (
            <div className="relative rounded-xl overflow-hidden aspect-video bg-gray-100">
              ...
              <button
                ...
                className="absolute bottom-2 right-2 px-3 py-1 bg-black/50 rounded-lg text-xs text-white hover:bg-black/70"
              >
                Change
              </button>
            </div>
        ...
        <div className="card p-6 space-y-4">
          <div>
            ...
            <input type="text" {...register('title')} className="input text-lg font-medium" />
          ...
          <div>
            ...
            <textarea {...register('content')} rows={14} className="input resize-y font-mono text-sm" />
          </div>
          <div>
